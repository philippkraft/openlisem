...
double TWorld::FindValue(double value, const  HORIZON *hor, int colv, int col)
{
    ...
    if (it == l->hydro[colv].begin()) {
        ...
        //int upperIndex = std::distance(l->hydro[colv].begin(), it);
        int upperIndex = lowerIndex + 1; // Index of upper bound (directly following lower bound)
        double lV = l->hydro[colv][lowerIndex]; // lower value
        double uV = l->hydro[colv][upperIndex]; // upper value
...
double TWorld::HNode(double theta,const  HORIZON *hor)
{
    ...
    if (it == l->hydro[THETA_COL].begin()) {
        ...
        int upperIndex = lowerIndex + 1;//std::distance(l->hydro[THETA_COL].begin(), it);     // Index of upper bound
...
double TWorld::DmcNode(double head, const  HORIZON *hor, bool on_dmch)
{
    ...
    if (on_dmch) {
        ...
        if (it == l->hydro[DMCH_COL].begin()) {
            ...
            int upperIndex = lowerIndex + 1;//std::distance(l->hydro[DMCH_COL].begin(), it);
        ...
        if (it == l->hydro[H_COL].begin()) {
            ...
            int upperIndex = lowerIndex + 1;//std::distance(l->hydro[H_COL].begin(), it);     // Index of upper bound
